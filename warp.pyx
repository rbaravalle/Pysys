...
cdef resampleExpand(np.ndarray[DTYPE_t, ndim=3] field, float u, float v, float w):
    cdef int a0,a1,b0,b1,c0,c1,suma
    a0 = floor(u)
    a1 = floor(u)+1
    b0 = floor(v)
...
def warpExpand(np.ndarray[DTYPE_t, ndim=3] field, np.ndarray[DTYPE_tf, ndim=3] gx, np.ndarray[DTYPE_tf, ndim=3] gy, np.ndarray[DTYPE_tf, ndim=3] gz, int N, int Nz, float k):
    cdef int x,y,z
    cdef float u,v,w,znew
    cdef np.ndarray[DTYPE_t, ndim=3] field2 = np.zeros((N,N,Nz),dtype=DTYPE)
    for x from 0<=x<N:    
        for y from 0<=y<N:
            for z from 0<=z<Nz:
                if(z > N/4 and z < N/2): znew = ((np.float(N)-np.float(y)/2.0)/np.float(N))
                else: znew = 1.0
                u = (x-gx[x,y,z]*k)*znew # f^(-1)
                v = (y-gy[x,y,z]*k)*znew
                w = (z-gz[x,y,z]*k)*znew
                try:
                    field2[x,y,z] = resample(field,u,v,w)#field[u,v,w]
...
# warp a 3D field using the gradient G(gx,gy,gz)
# expand the total volume to simulate baking
def warpExpandGeom(np.ndarray[DTYPE_t, ndim=3] field, np.ndarray[DTYPE_tf, ndim=3] gx, np.ndarray[DTYPE_tf, ndim=3] gy, np.ndarray[DTYPE_tf, ndim=3] gz, int N, int Nz, float k):
    cdef int x,y,z
    cdef float u,v,w,znew
    cdef np.ndarray[DTYPE_t, ndim=3] field2 = np.zeros((N,N,Nz),dtype=DTYPE)
    for x from 0<=x<N:    
