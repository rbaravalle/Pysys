...
dx=0.001        # Interval size in x-direction.
...
def updatefig(*args):
    ...
    I = Image.frombuffer('L',(nx,ny),  255*(255*np.asarray(ui) > 170).astype(np.uint8) ,'raw','L',0,1)
...
fig = plt.figure(1)
img = subplot(111)
im = img.imshow( ui, cmap=cm.gray, interpolation='nearest', origin='lower')
